...
def knn_weights(gdf, k=13): # KNN (k-nearest neighbors) - based on closest points
    coords = [(geom.centroid.x, geom.centroid.y) for geom in gdf.geometry] 
    return KNN(coords, k=k) 

def distance_weights(gdf, threshold=30): # DistanceBand - based on distance threshold
    coords = [(geom.centroid.x, geom.centroid.y) for geom in gdf.geometry]
